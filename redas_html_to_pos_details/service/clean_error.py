...
def save_html_log(html, platform, spiderUuid, url, e):
    ...
    if is_ture:
        src_url_pos_id = re.search(pattern, url).group(1).strip().replace("/", "_").replace("?", "_")
        ...
        # 将出错的html写入文件
        file_url_name = html_path + "/" + src_url_pos_id + "_" + time_ticks + ".html"
